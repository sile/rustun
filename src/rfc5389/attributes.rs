...
use attribute::{RawAttribute, Type};
use message::{Message, RawMessage};
use rfc5389::errors;
use types::SocketAddrValue;
use {Attribute, ErrorKind, Result};
...
impl MessageIntegrity {
    ...
    fn calc_hmac_sha1(key: &[u8], preceding: &RawMessage) -> [u8; 20] {
        ...
        (&mut bytes[2..4])
            .write_u16be(adjusted_len as u16)
            .expect("must succeed");
...
mod test {
    use super::*;
    use message::{self, Message, RawMessage};
